...
import { NarrativeScheduler, createStoryProgress } from '../src/narrative/narrative-scheduler';
...
describe('NarrativeScheduler', () => {
  ...
  it('does not evaluate ending if already achieved', () => {
    ...
    // Override the endingAchieved after state creation to simulate already having an ending
...
describe('LetterScheduler', () => {
  ...
  it('selects mandatory letter first', () => {
    ...
    const deliveries = letterScheduler.selectNextLetters(state, saveData, 1);
  ...
  it('prioritizes chapter-critical letters for relationship trajectory', () => {
    ...
    // Should prioritize fenwick_memory_promise (care > 0.6)
    expect(deliveries.length).toBe(1);
    expect(deliveries[0].letterId).toContain('fenwick');
  });
  
  it('prioritizes café letters for comfort trajectory', () => {
    const state = createMockState({ 
      chapter: 2,
  ...
  it('prioritizes mystery letters for curiosity trajectory', () => {
    ...
    expect(deliveries.length).toBe(1);
    // Should prioritize wren_first_clue_letter
  ...
  it('never selects already consumed letter', () => {
    ...
    // marigold_ch0_welcome is consumed and already delivered
    const eligible = letterScheduler.getEligibleLetters(state, saveData);
  ...
  it('respects max per chapter per source limit (3)', () => {
    ...
    const saveData = createMockSaveData({ day: 6 });
    
    // Request many letters to test cap
    const deliveries = letterScheduler.selectNextLetters(state, saveData, 10);
  ...
  it('filters by chapter - no future chapter letters', () => {
    const state = createMockState({ chapter: 1 });
    const saveData = createMockSaveData({ day: 5 });
  ...
  it('returns identical results for identical state (deterministic)', () => {
    ...
    const deliveries1 = letterScheduler.selectNextLetters(state, saveData, 2);
    const deliveries2 = letterScheduler.selectNextLetters(state, saveData, 2);
